...
This repository is organized as a monorepo so the player client, shared types, and future packages (API, workers, etc.) can evolve together. Package layout and tooling will be added as implementation begins.
...
│   ├── api/                    # NestJS API
│   ├── client/                 # Player-facing React app (Vite)
...
Authentication uses [Clerk](https://clerk.com/) in `apps/client` and `apps/admin` (same Clerk application). After cloning:

- **Client:** copy `apps/client/.env.example` → `apps/client/.env.local` and set `VITE_CLERK_PUBLISHABLE_KEY` (`clerk env pull` from `apps/client` fills this; do not use the secret key in the client).
...
- **API:** copy `apps/api/.env.example` → `apps/api/.env` and set `CLERK_SECRET_KEY` (copy from Dashboard or from client `.env.local` after `clerk env pull` — keep it out of the Vite bundle).
...
| `npm run build`     | Production build (client + API)                     |
...
| `task format:check`          | Check formatting without writing (whole repo)                                                                  |
...
| `task <project>:ci`          | Per-project format-check, lint, test, and build (`schemas`/`ui`/`etl`/`api`/`client`/`admin`/`infra`/`shared`) |
