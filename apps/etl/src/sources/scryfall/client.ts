...
export async function fetchBulkMetadata(
  ...
  if (!response.ok) {
    throw new Error(
      `Scryfall bulk-data HTTP ${response.status}: ${await response.text()}`,
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      `Unexpected Scryfall bulk-data document: ${parsed.error.message}`,
    );
  }
...
export function selectBulkDataset(
  ...
  if (!match) {
    ...
    throw new Error(
      `Scryfall bulk type "${type}" not found. Available: ${available}`,
    );
  }
  return match;
}
