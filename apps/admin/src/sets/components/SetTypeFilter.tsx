...
import { SET_SEARCH_SET_TYPE_MAX } from '@respark/schemas';

import { useSetTypeSuggestions } from '../hooks/sets';
...
// a filter for magic set types (IE expansion, commander, etc)
export const SetTypeFilter = ({ value, onChange }: SetTypeFilterProps) => {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setDebouncedQ(draft.trim()), 200);
    return () => window.clearTimeout(timer);
  }, [draft]);

  const { data } = useSetTypeSuggestions(debouncedQ, !atLimit);
  const suggestions = (data ?? []).filter((hit) => !hasType(value, hit));
  ...
  useEffect(() => {
    ...
    const next = (data ?? []).filter((hit) => !hasType(value, hit));
