...
export async function fetchEtlSync(getToken: GetToken, id: string): Promise<EtlSync> {
  const body = await apiFetchJson<{ sync: EtlSync }>(`/admin/etl-syncs/${id}`, getToken);
  return body.sync;
}
...
export async function fetchJobUnmatched(
  ...
  return apiFetchJson<{ unmatched: IngestionUnmatched[]; total: number }>(path, getToken);
}
