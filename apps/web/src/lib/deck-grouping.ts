...
export function groupDeckCards(cards: DeckCard[], mode: DeckGroupMode): DeckCardGroup[] {
  if (mode === 'none') {
    return [
      {
        key: 'all',
...
function bucketBy(cards: DeckCard[], keyOf: (card: DeckCard) => string): Map<string, DeckCard[]> {
