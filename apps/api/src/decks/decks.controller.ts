...
export class DecksController {
  ...
  async patchCard(
    ...
    if (!hasQuantity && !hasPrinting && !hasFoil && !hasSideboard) {
      throw new BadRequestException('quantity, printingId, foil, or sideboard is required');
    }

    let card = null as Awaited<ReturnType<DecksService['setCardPrinting']>> | null;
    let lineId = deckCardId;

    if (hasPrinting) {
      const printingId = typeof body.printingId === 'string' ? body.printingId : '';
      requireUuid(printingId, 'printingId');
      card = await this.decksService.setCardPrinting(userId, id, lineId, printingId);
      lineId = card.id;
    }
    ...
    if (hasSideboard) {
      ...
      card = await this.decksService.setCardSideboard(userId, id, lineId, body.sideboard);
      lineId = card.id;
    }
    ...
    if (hasQuantity) {
      ...
      card = await this.decksService.setCardQuantity(userId, id, lineId, body.quantity);
    }
