...
export const getDeckTool = (decks: DecksService): ChatTool<GetDeckInput, CompactDeck> => ({
  name: 'getDeck',
  description:
    'Load a deck the player owns (format, commander with oracle/keywords, compact lines, type/curve/keyword stats). Use commander oracle and stats.keywordCounts when choosing searchCards q or typeContains; do not search with an empty q when the list has a theme. Uses the sticky deck when no id is passed. Passing an owned id attaches that deck for later turns.',
