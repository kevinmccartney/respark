...
- **Pipeline schemas** — `raw`, `catalog`, `market`, `app`, plus `ops.etl_sync` / `ops.etl_job_run` for ETL sync tracking. See [`apps/etl/README.md`](../etl/README.md) and `task etl -- --help`.
