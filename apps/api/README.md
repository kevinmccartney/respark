...
- **`GET /cards`** — authenticated search over unique `catalog.card` rows (`page` + `limit` pagination; optional `q` keyword ILIKE, `scryfall` Scryfall-syntax query parsed locally via `packages/scryfall-query` and compiled to SQL, `legalIn`, `colorIdentity`, `commanderEligible`, `typeContains`, `maxManaValue`, `excludeCardIds`). Supported Scryfall sections: colors/identity, types, oracle/keywords, mana/mv/devotion/produces, rarity, sets/blocks/group/`in`/`st`/`cn`, plus `is:`/`has:`/`new:` flags in those sections. Unknown or out-of-scope keywords return **400** with a clear message. Compose with other filters via AND.
